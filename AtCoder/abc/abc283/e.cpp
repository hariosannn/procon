...
int main() {
    ...
    rep(i, 2) rep(j, 2) dp.at(0).at(i).at(j) = i+j;
        ...
        for(ll flip_2=0; flip_2<2; flip_2++){
            for(ll flip_1=0; flip_1<2; flip_1++){
                for(ll flip_0=0; flip_0<2; flip_0++){
                    V_L A0 = tmp_A.at(0), A1 = tmp_A.at(1), A2 = tmp_A.at(2);
                    ...
                    if(!is_isolate(check_A)){
                        if(flip_2) chmin(dp.at(i).at(flip_1).at(flip_2), dp.at(i-1).at(flip_0).at(flip_1) + 1);
                        else chmin(dp.at(i).at(flip_1).at(flip_2), dp.at(i-1).at(flip_0).at(flip_1));
                    }
                }
            }
        }
