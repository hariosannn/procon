...
mint inversion_num(V_L &A){
    ...
    SEGMENT_TREE<mint> seg_t(seg_len);

    mint inv = 0;
    rep(i, N){
        inv += seg_t.sum(0, A.at(i))/iterative_square_mod(2, -(i-N-1));
        seg_t.add(A.at(i), iterative_square_mod(2, N-i));
    }
    return inv;
}
