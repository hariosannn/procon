...
// index が条件を満たすかどうか
bool isOK(ll index, ll key, vector<ll> &a, ll m, map<pair<ll, ll>, ll> &add_map) {
    if ((a[index] + m*(add_map[pair<ll, ll>{index, a[index]}])) >= key) return true;
    else return false;
}

// 汎用的な二分探索のテンプレ
ll binary_search(ll key, vector<ll> &a, ll m, map<pair<ll, ll>, ll> &add_map) {
    ...
    while (abs(ok - ng) > 1) {
        ...
        if (isOK(mid, key, a, m, add_map)) ok = mid;
        else ng = mid;
    }
    return ok;
}
...
int main(){
    ...
    map<pair<ll, ll>, ll> add_map;
    rep(i, N){
        add_map[pair<ll, ll>{i, A.at(i)}] = i+1;
    }
    sort(A.begin(), A.end());
    rep(i, M){
        ll ans = binary_search(0, A, i+1, add_map);
        cout  << ans << endl;
    }
}